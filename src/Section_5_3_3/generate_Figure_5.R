...
#   simulations_derandomised_FDR_lsf.sh
...
for (model in c("gaussian", "logistic")){ 
  ...
  # saveRDS(results, paste0("./results/results_derandomised_",model,".rds"))
  
   results <- readRDS(paste0("./results/results_derandomised_",model,".rds"))
    ...
    summary_df_filtered <- summary_df  |>
      dplyr::filter(p_relevant %in% c(20, 80)) |>
      mutate(p_relevant_lab = factor(p_relevant,
                                     levels = c(20, 80),
                                     labels = c("p[relevant]==20", "p[relevant]==80")))
    } else{
      summary_df_filtered <- summary_df  |>
        dplyr::filter(p_relevant %in% c(15, 60)) |>
        mutate(p_relevant_lab = factor(p_relevant,
                                       levels = c(15, 60),
                                       labels = c("p[relevant]==15", "p[relevant]==60")))
    }
    ...
    amplitude_power <- ggplot(
      ...
      geom_point(size = 6) +
      scale_color_manual(
        values = c("Original derandomised FDR" = "#E41A1C",  "Posthoc derandomised FDR" = "#377EB8"),
        labels = c(expression("Original derandomised FDR KO"),  expression("Posthoc derandomised FDR KO"))
      ) +
      scale_shape_manual(
        values = c("Original derandomised FDR" = 16,  "Posthoc derandomised FDR" = 17),
        labels = c(expression("Original derandomised FDR KO"),  expression("Posthoc derandomised FDR KO"))
      ) +
      labs(
    ...
    base_plot <- ggplot(plot_df, aes(
      ...
      geom_line(size = 2) +
      geom_point(size = 6) +
      scale_color_manual(values = c("Original derandomised FDR" = "#E41A1C",  "Posthoc derandomised FDR" = "#377EB8"),
                         labels = list(expression("Original derandomised FDR KO"),  expression("Posthoc derandomised FDR KO"))) +
      scale_shape_manual(values = c("Original derandomised FDR" = 16,  "Posthoc derandomised FDR" = 17),
                         labels = list(expression("Original derandomised FDR KO"),   expression("Posthoc derandomised FDR KO"))) +
      ...
      scale_y_continuous(
        breaks = seq(0, 0.4, by = 0.2),  # ticks every 0.2
        limits = c(-0.03, 0.45),              # ensure axis includes 1
        expand = expansion(mult = 0)   # avoid extra padding
      ) + 
    ...
    ggsave(
      filename = paste0(model,"_derandomised_FDR_power.pdf") ,
      plot = amplitude_power,
      device = "pdf",
      path = "./figures/",
      width = 13, height = 5.5, units = "in",  scale = 0.8
    )
    ggsave(
      filename = paste0(model,"_derandomised_FDR_FDR.pdf") ,
      plot = amplitude_FDR,
      device = "pdf",
      path = "./figures/",
      width = 13, height = 5.5, units = "in",  scale = 0.8
    ...
    ggsave(
      filename = paste0( model,"_derandomised_FDR_ratio.pdf") ,
