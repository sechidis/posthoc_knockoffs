...
for (model in models){ 
  ...
  summary_df_filtered <- summary_df  |>
    dplyr::filter(p_relevant == p_relevant_fixed)
  ...
  plot_df <- summary_df_filtered  |>
    ...
    mutate(metric = recode(metric, avg_FDP = "FDR", avg_alpha = "alpha"))
  ...
  ggsave(
    filename = paste0( model,"_original_ampl_power.pdf") ,
    plot = amplitude_power,
    device = "pdf",
    path = "./figures/",
    width = 6.5, height = 4.2, units = "in",  scale = 1
  )
  
  ggsave(
    filename = paste0( model,"_original_ampl_FDR.pdf") ,
    plot = amplitude_FDR,
    device = "pdf",
    path = "./figures/",
    width = 6.5, height = 4.2, units = "in",  scale = 1
  ...
  ggsave(
    filename = paste0( model,"_original_ampl_ratio.pdf") ,
  ...
  # Filter results to plut 
  summary_df_filtered <- summary_df  |>
    dplyr::filter(amplitude == amplitude_fixed & p_relevant>=2 )
  ...
  ggsave(
    filename = paste0( model,"_original_p_relevant_power.pdf") ,
  ...
  ggsave(
    filename = paste0( model,"_original_p_relevant_FDR.pdf") ,
    plot = p_relevant_FDR,
    device = "pdf",
    path = "./figures/",
    width = 6.5, height = 4.2, units = "in",  scale = 1
  ...
  ggsave(
    filename = paste0( model,"_original_p_relevant_ratio.pdf") ,
    plot = p_relevant_ratio,
    device = "pdf",
    path = "./figures/",
    width = 6.5, height = 4.2, units = "in",  scale = 1
  )
